...
def g1(arr):  # 2 features a1 a2
    return noteq(arr[0], arr[1])


def g2(arr):  # XOR combination 3 features a1 a2 a3
    return noteq(noteq(arr[0], arr[1]), arr[2])


def g3(arr):  # 6 features
...
def g4(arr):  #  NAND(a0, a2)  # 2 features a1 a2
    if arr[0] == 1 and arr[2] == 1:
        return 0
    return 1


def g5(arr):  # 4 features combined with g4: a1 a2 a29 a30
    return noteq(g4(arr), noteq(arr[-1], arr[-2]))


def g6(
    arr,
):  # using 10 attributes arr[9] to arr[18] - 10 middle features - a third of 30
    return noteq(
        noteq(
...
def g7(arr):  # using all attributes - all of them have the same FRI
    if sum(arr) == 3:
        return 1
    return 0
